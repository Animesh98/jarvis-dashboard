  ...
  while ((match = regex.exec(title)) !== null) {
    ...
    if (entity?.tmdb_id) {
      ...
      parts.push(
        <Link
          key={i++}
          href={`/discover/${eType}/${entity.tmdb_id}`}
...
function EditTaskForm({
  ...
  task: Task;
  onSave: (id: string, title: string, entities: Entity[]) => void;
  onCancel: () => void;
}) {
      ...
      if (query.length >= 2) {
        ...
        debounceRef.current = setTimeout(async () => {
          const r = await api<{ results: any[] }>(
            `/api/recommendations/autocomplete?q=${encodeURIComponent(query)}`
          );
          if (r.data?.results) {
  ...
  const insertEntity = (item: any) => {
    ...
    setEntities((prev) => [
      ...prev,
      { name: item.title, type: item.type || 'movie', tmdb_id: item.tmdb_id || '' },
    ]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (showSuggestions && suggestions.length > 0) {
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        insertEntity(suggestions[activeIdx]);
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if (value.trim()) onSave(task.id, value.trim(), entities);
    }
  ...
  return (
    <div className={styles.editForm} ref={formRef} onClick={(e) => e.stopPropagation()}>
      ...
      <div className={styles.editActions}>
        ...
        <button
          className={`${styles.editActionBtn} ${styles.editActionSave}`}
          onClick={() => {
            if (value.trim()) onSave(task.id, value.trim(), entities);
          }}
          title="Save"
...
function AddTaskForm({
  ...
  column: string;
  onAdd: (title: string, entities: Entity[]) => void;
  onCancel: () => void;
}) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if (value.trim()) onAdd(value.trim(), entities);
    }
    if (e.key === 'Escape') onCancel();
  };
  ...
  return (
    <div className={styles.addForm} ref={formRef}>
      ...
      <div className={styles.addFormActions}>
        <button className={`btn btn-ghost btn-sm ${styles.addFormBtn}`} onClick={onCancel}>
          Cancel
        </button>
        <button
          className={`btn btn-primary btn-sm ${styles.addFormBtn}`}
          onClick={() => {
            if (value.trim()) onAdd(value.trim(), entities);
          }}
        >
...
export default function TasksPage() {
  ...
  useEffect(() => {
    ...
    const today = toLocalDate();
    api<{ migrated: number }>(`/api/tasks/migrate?date=${today}`, { method: 'POST' }).then((r) => {
      if (r.data?.migrated) setMigratedCount(r.data.migrated);
    });
  }, []);
  ...
  const tasksInColumn = useCallback(
    (col: string) => tasks.filter((t) => t.column === col).sort((a, b) => a.order - b.order),
    [tasks]
  );
  ...
  const handleCopyTask = (task: Task) => {
    copyToClipboard(plainTitle(task.title))
      .then(() => {
    ...
    <>
      <header className="page-header">
        ...
        <div className={styles.datePicker}>
          ...
          {!isToday && (
            <button className={styles.dateTodayBtn} onClick={() => setSelectedDate(toLocalDate())}>
              Today
            </button>
          )}
        </div>
      ...
      {migratedCount > 0 && isToday && (
        <div className={styles.migrateBanner}>
          Carried over {migratedCount} incomplete {migratedCount === 1 ? 'task' : 'tasks'} from
          previous days
          <button className={styles.migrateDismiss} onClick={() => setMigratedCount(0)}>
            &times;
          </button>
        </div>
      )}
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              <div key={col.id} className={`${styles.column} ${styles[`col_${col.id}`]}`}>
                ...
                {addingTo !== col.id && (
                  <button className={styles.addBottomBtn} onClick={() => setAddingTo(col.id)}>
